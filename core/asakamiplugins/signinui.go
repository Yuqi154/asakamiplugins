...
func drawScore16(a *scdata) (image.Image, error) {
	...
	canvas.DrawStringAnchored(fmt.Sprintf("+%02.2f稻荷币", a.dhbinc), 350, 350, 0, 0)
	canvas.DrawStringAnchored(fmt.Sprintf(" %02.2f円", a.jpyinc), 350, 400, 0, 0)
	...
	canvas.DrawStringAnchored("Created By Zerobot-Plugin "+banner.Version+" & Asakami-Plugin"+signinpluginversion, float64(imgDX)/2, float64(imgDY)-20, 0.5, 0.5) // zbp
	canvas.SetRGB255(0, 0, 0)
	canvas.DrawStringAnchored("Created By Zerobot-Plugin "+banner.Version+" & Asakami-Plugin"+signinpluginversion, float64(imgDX)/2-3, float64(imgDY)-19, 0.5, 0.5) // zbp
	canvas.SetRGB255(255, 255, 255)
	// Gradient
...
func drawScore15(a *scdata) (image.Image, error) {
	...
	canvas.DrawString(a.nickname+fmt.Sprintf("+%02.2f稻荷币", a.dhbinc), float64(back.Bounds().Size().X)*0.1, float64(back.Bounds().Size().Y)*1.3)
	canvas.DrawString(fmt.Sprintf("+%02.2f円", a.jpyinc), float64(back.Bounds().Size().X)*0.1, float64(back.Bounds().Size().Y)*1.4)
...
func drawScore17(a *scdata) (image.Image, error) {
	...
	createAeroBox(20, float64(imgDY-150), 280, 130)               // left bottom
	...
	canvas.DrawStringAnchored(fmt.Sprintf("连续签到%d天", a.continuoussignin), 40, float64(imgDY-120), 0, 0)
	canvas.DrawStringAnchored(fmt.Sprintf("+%02.2f稻荷币", a.dhbinc), 40, float64(imgDY-90), 0, 0)
	canvas.DrawStringAnchored(fmt.Sprintf("+%02.2f円", a.jpyinc), 40, float64(imgDY-60), 0, 0)
...
func customtext(a *scdata, fontdata []byte, cw, ch, aw float64, textcolor color.Color) (img image.Image, err error) {
	...
	canvas.DrawStringAnchored("Create By ZeroBot-Plugin "+banner.Version+" & Asakami-Plugin"+signinpluginversion, 0+4, ch, 0, -0.5)
	...
	canvas.DrawStringAnchored(fmt.Sprintf("+%02.2f稻荷币", a.dhbinc), ((cw-scw)-(cw/3-scw/2))/8, (ch-sch)/2+sch/4+tempfh, 0, 0.5)
	...
	canvas.DrawStringAnchored(fmt.Sprintf("+%02.2f円", a.jpyinc), ((cw-scw)-(cw/3-scw/2))/8, (ch-sch)/2+sch/4*3, 0, 0.5)
