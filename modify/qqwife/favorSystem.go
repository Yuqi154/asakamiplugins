...
import (
	...
	"github.com/FloatTech/ZeroBot-Plugin/plugin/asakamiplugins"
...
var backpack = asakamiplugins.Getbackpack()
var link = asakamiplugins.Getlink()
// 好感度系统
...
func init() {
	...
	engine.OnRegex(`^买礼物给\s?(\[CQ:at,qq=(\d+)\]|(\d+))`, getdb).SetBlock(true).Limit(ctxext.LimitByUser).
		Handle(func(ctx *zero.Ctx) {
			...
			// 记录结果
			dhcitem.Quantity -= float64(moneyToFavor)
