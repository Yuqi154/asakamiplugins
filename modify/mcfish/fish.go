...
func init() {
	engine.OnRegex(`^进行(([1-5]\d|[1-9])次)?钓鱼$`, getdb).SetBlock(true).Limit(ctxext.LimitByUser).Handle(func(ctx *zero.Ctx) {
		...
		if equipInfo == (equip{}) {
			...
			// 等待用户下一步选择
			recv, cancel := zero.NewFutureEvent("message", 999, false, zero.RegexRule(`^(是|否)$`), zero.CheckUser(link.Getrealid(ctx))).Repeat()
			defer cancel()
			buy := false
			for {
				select {
				...
				case e := <-recv:
					...
					dycitem, err := backpack.GetItem(1002, uid)
					if err != nil {
						return
					}
					money := dycitem.Quantity
					if money < 100 {
						dhcitem, err := backpack.GetItem(1001, uid)
						if err != nil {
							return
						}
						if dhcitem.Quantity > 1 {
							ctx.SendChain(message.Text("你钱包当前只有", money, "钓鱼币,是否花费1钓鱼券兑换100钓鱼币?\n回答\"是\"或\"否\""))
							// 等待用户下一步选择
							recv, cancel := zero.NewFutureEvent("message", 999, false, zero.RegexRule(`^(是|否)$`), zero.CheckUser(link.Getrealid(ctx))).Repeat()
							defer cancel()
							for {
								...
								case e := <-recv:
									...
									dhcitem.Quantity--
									backpack.UpdateItem(dhcitem, uid)
									if err != nil {
										ctx.SendChain(message.Text("[ERROR at fish.go.3]:", err))
										return
									}
									dycitem.Quantity += 100
									backpack.UpdateItem(dycitem, uid)
									if err != nil {
										ctx.SendChain(message.Text("[ERROR at fish.go.3]:", err))
										return
									}
							...
							ctx.SendChain(message.Text("你钱包当前只有", money, "钓鱼币,无法完成支付"))
							return
						}
					}
					dycitem.Quantity -= 100
