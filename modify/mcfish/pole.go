...
func init() {
	...
	engine.OnRegex(`^合成(.+竿|三叉戟)$`, getdb).SetBlock(true).Limit(ctxext.LimitByUser).Handle(func(ctx *zero.Ctx) {
		...
		if len(articles) > 3 {
			...
			ctx.Send(message.ReplyWithMessage(ctx.Event.MessageID, msg...))
			// 等待用户下一步选择
			recv, cancel := zero.NewFutureEvent("message", 999, false, zero.RegexRule(`^(取消|\d+ \d+ \d+)$`), zero.CheckUser(link.Getrealid(ctx))).Repeat()
			defer cancel()
			for {
				select {
				case <-time.After(time.Second * 120):
				...
				case e := <-recv:
					...
					if first == second || first == third || second == third {
						ctx.SendChain(message.At(link.Getrealid(ctx)), message.Text("[0]请输入正确的序号\n", list))
						continue
					}
					if first > max || second > max || third > max {
						ctx.SendChain(message.At(link.Getrealid(ctx)), message.Text("[", max, "]请输入正确的序号\n", list))
						continue
					}
					check = true
				}
