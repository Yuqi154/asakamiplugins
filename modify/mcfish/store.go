...
func limitSet(ctx *zero.Ctx) *rate.Limiter {
	return storeLimiter.Load(link.Getrealid(ctx))
}
...
func init() {
	...
	engine.OnRegex(`^出售(`+strings.Join(thingList, "|")+`)\s*(\d*)$`, getdb, refreshFish).SetBlock(true).Limit(limitSet).Handle(func(ctx *zero.Ctx) {
		...
		if len(articles) > 1 {
			...
			// 等待用户下一步选择
			sell := false
			recv, cancel := zero.NewFutureEvent("message", 999, false, zero.RegexRule(`^(取消|\d+)$`), zero.CheckUser(link.Getrealid(ctx))).Repeat()
			defer cancel()
			for {
				select {
				...
				case e := <-recv:
					...
					if err != nil || index > len(articles)-1 {
						ctx.SendChain(message.At(link.Getrealid(ctx)), message.Text("请输入正确的序号"))
						continue
					}
					sell = true
				}
		...
		// 等待用户下一步选择
		recv, cancel1 := zero.NewFutureEvent("message", 999, false, zero.RegexRule(`^(是|否)$`), zero.CheckUser(link.Getrealid(ctx))).Repeat()
		defer cancel1()
		buy := false
		for {
			select {
		...
		if len(records) != 0 {
			...
			if numberOfRecord > 0 {
				...
				// 等待用户下一步选择
				recv, cancel2 := zero.NewFutureEvent("message", 999, false, zero.RegexRule(`^(是|否)$`), zero.CheckUser(link.Getrealid(ctx))).Repeat()
				defer cancel2()
				use := false
				checkTime := false
				for {
		...
		pice = pice * 8 / 10
		dhbitem, err := backpack.GetItem(1002, uid)
		if err != nil {
			return
		}
		dhbitem.Quantity += float64(pice * number)
	...
	engine.OnRegex(`^购买(`+strings.Join(thingList, "|")+`)\s*(\d*)$`, getdb, refreshFish).SetBlock(true).Limit(limitSet).Handle(func(ctx *zero.Ctx) {
		...
		money := dhbitem.Quantity
		if money < float64(price) {
		...
		dhbitem.Quantity -= float64(price)
		backpack.UpdateItem(dhbitem, uid)
		if err != nil {
